...
const Footer = () => {
  ...
  useEffect(() => {
    fetchSettings().then(data => { if (data?.lastUpdated) setLastUpdated(data.lastUpdated); });
  }, []);

  useEffect(() => {
    const handleUpdate = () => fetchSettings().then(data => { if (data?.lastUpdated) setLastUpdated(data.lastUpdated); });
    window.addEventListener('portfolio_admin_update', handleUpdate);
    return () => window.removeEventListener('portfolio_admin_update', handleUpdate);
  }, []);
...
<div className="flex flex-col items-center md:items-start space-y-2 md:flex-1">
            <div className="text-sm" style={{ color: isDarkMode ? themeColors.colors.white : themeColors.colors.pink[700] }}>
              © {new Date().getFullYear()} Bui Tong Giang. All rights reserved.
            </div>
            <div className="text-xs" style={{ color: isDarkMode ? 'rgba(253,213,223,0.45)' : 'rgba(180,100,120,0.55)' }}>
